...
if (isset($_POST['messageContent'])) {
    ...
    if (!isset($_POST["coversationId"])) {
        ...
        FROM conversation
        WHERE (participant_user1 = '$sender' AND participant_user2 = '$receiver')
        OR (participant_user1 = '$receiver' AND participant_user2 = '$sender') ";
        $oldConversation = [];
        ...
        if (mysqli_num_rows($checkConversationResult) > 0) {
            ...
            echo json_encode(array("status" => 200, "data" => "conversation not exist"));
            if (isset($_FILES["image"]) && $_FILES["image"] != null) {
                $uniqueImageName = uniqid("IMG-") . $_FILES["image"]["name"];
                $target_file = $targetDirectory . basename($uniqueImageName);
                if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
